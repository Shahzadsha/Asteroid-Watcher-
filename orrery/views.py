...
def nasa_data_logs(request):
    """Displays the NASA data logs and the full Celestial Bodies Database."""
    logs = NasaDataLog.objects.all().order_by('-timestamp')
    
    # Fetch all celestial bodies to display the database
    planets = Planet.objects.all()
    comets = Comet.objects.all()
    asteroids = Asteroid.objects.all()
    ...
    return render(request, 'orrery/nasa_data_logs.html', {
        'logs': logs,
        'celestial_bodies': celestial_bodies
    })
